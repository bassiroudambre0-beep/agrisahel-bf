...
const Select = ({ label, options, ...props }) => (
  <div style={{ marginBottom: 16 }}>
    {label && <label style={G.label}>{label}</label>}
    <select style={{ ...G.input, appearance: "none", width: "100%", maxWidth: "100%" }} {...props}>
...
const AuthPage = ({ onAuth }) => {
  ...
  if (step === "otp") return (
    <div style={{ minHeight: "100vh", background: `linear-gradient(160deg,${COLORS.primary} 0%,${COLORS.primary2} 100%)`, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: COLORS.white, borderRadius: 28, padding: 24, width: "100%", maxWidth: "100%", boxSizing: "border-box", overflow: "hidden" }} className="slide-up">
      ...
      <nav style={{
        ...
        <div style={{ display:"flex", justifyContent:"space-around", alignItems:"center", padding:"4px 0" }}>
          {TABS.slice(0, 5).map(t => (
            <button key={t.id} onClick={() => { onChange(t.id); setShowMore(false); }}
              style={{ background:"none", border:"none", flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:"3px", color: active===t.id ? COLORS.primary : COLORS.gray, fontSize:"11px", fontWeight: active===t.id ? "800" : "600", padding:"6px 4px", borderRadius:"10px", cursor:"pointer" }}>
              <span style={{ fontSize: active===t.id ? "26px" : "22px" }}>{t.icon}</span>
              <span>{t.label}</span>
            </button>
          ))}
          <button onClick={() => setShowMore(p => !p)}
            style={{ background: showMore ? COLORS.primary+"15" : "none", border:"none", flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:"3px", color:COLORS.gray, fontSize:"11px", fontWeight:"600", padding:"6px 4px", borderRadius:"10px", cursor:"pointer" }}>
...
export default function AgriSahelBF() {
  ...
  return (
    <>
      <GlobalStyle />
      <ToastContainer />
      <div style={{ maxWidth: "480px", margin: "0 auto", minHeight: "100vh", background: COLORS.cream, paddingBottom: "110px", position: "relative", overflowX: "hidden", boxSizing: "border-box" }}>
