 ...
 // ════════════════════════════════════════════════════════
 const COLORS = {
   primary: "#1B4332", primary2: "#2D6A4F", primary3: "#52B788",
   amber: "#D4A017", amber2: "#F4CC55",
   cream: "#FDF6EC", cream2: "#F0EBE1",
 ...
 const G = {
   ...
   label: { fontSize: 13, fontWeight: 700, color: COLORS.gray, marginBottom: 6, display: "block" },
 ...
 // UI COMPONENTS
 // ════════════════════════════════════════════════════════
 const Card = ({ children, style = {} }) => <div style={{ ...G.card, ...style }}>{children}</div>;
 ...
 const GlobalStyle = () => (
   <style>{`
     ...
     * { box-sizing: border-box; margin: 0; padding: 0; }
     ...
     input:focus, select:focus, textarea:focus { border-color: #2D6A4F !important; box-shadow: 0 0 0 3px rgba(45,106,79,0.15); }
   `}</style>
 );
 ...
 const AuthPage = ({ onAuth }) => {
   ...
   if (step === "otp") return (
     <div style={{ minHeight: "100vh", background: `linear-gradient(160deg,${COLORS.primary} 0%,${COLORS.primary2} 100%)`, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
       <div style={{ background: COLORS.white, borderRadius: 28, padding: 32, width: "100%", maxWidth: 400 }} className="slide-up">
 ...
 const BottomNav = ({ active, onChange }) => {
   ...
   return (
     <>
       ...
       <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 50, background: COLORS.white, borderTop: `1px solid ${COLORS.cream2}`, display: "flex", padding: "8px 4px 12px" }}>
         {mainTabs.map(t => (
 ...
 const WeatherWidget = ({ ville }) => {
   ...
   return (
     <Card style={{ background: `linear-gradient(135deg,${COLORS.primary},${COLORS.primary2})`, color: COLORS.white }}>
       ...
       {!validKey && <div style={{ marginTop: 10, fontSize: 11, opacity: 0.6, textAlign: "center" }}>Mode démo — Configurez VITE_OPENWEATHER_KEY pour la vraie météo</div>}
     </Card>
   );
 };
 ...
 export default function AgriSahelBF() {
   ...
   return (
     <>
       <GlobalStyle />
       <ToastContainer />
       <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: COLORS.cream }}>
